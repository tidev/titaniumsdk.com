...
for (const m of collected) {
  ...
  const index: ModuleIndex = {
    schemaVersion: SCHEMA_VERSION,
    moduleId: m.moduleId,
    // No display name. A module's manifest `name` is a build label that nothing
    // reads: node-appc assigns it once from package.json and never looks at it,
    // the install directory is the moduleid (`modules/android/ti.map/5.7.0`),
    // and `ti module` lists by id despite calling its loop variable `name`.
    // Carrying it here would only invite it to diverge from the id developers
    // actually write in tiapp.xml.
